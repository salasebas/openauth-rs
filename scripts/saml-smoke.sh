#!/usr/bin/env bash
# SAML smoke helper for openauth-sso / openauth-saml.
#
...
#
# See crates/openauth-sso/SMOKE-SAML.md for the full manual checklist.

set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SMOKE_DOC="${ROOT}/crates/openauth-sso/SMOKE-SAML.md"
ENV_EXAMPLE="${ROOT}/crates/openauth-sso/.env.saml-smoke.example"

red() { printf '\033[0;31m%s\033[0m\n' "$*"; }
green() { printf '\033[0;32m%s\033[0m\n' "$*"; }
yellow() { printf '\033[1;33m%s\033[0m\n' "$*"; }

echo "openauth SAML smoke"
...
echo "Running openauth-sso SAML integration tests..."
cargo test -p openauth-sso --features saml,oidc -- saml
green "openauth-sso SAML tests passed"

echo
echo "Running openauth-saml (saml-signed) unit/security tests..."
cargo test -p openauth-saml --features saml-signed
green "openauth-saml tests passed"
...
if [[ "${SAML_SMOKE_LIVE:-}" != "1" ]]; then
  ...
  yellow "Live smoke requires a running OpenAuth server and a real Okta/Azure/Google SAML app."
  exit 0
fi
...
if curl -fsS "${METADATA_URL}" -o /tmp/openauth-saml-sp-metadata.xml; then
  green "  SP metadata reachable: ${METADATA_URL}"
  if grep -q "EntityDescriptor" /tmp/openauth-saml-sp-metadata.xml; then
...
if [[ -f "${cert_path}" ]]; then
  ...
  printf '%s\n' "${SAML_SMOKE_IDP_CERT_PEM}" > /tmp/openauth-saml-idp-cert.pem
  cert_path="/tmp/openauth-saml-idp-cert.pem"
else
