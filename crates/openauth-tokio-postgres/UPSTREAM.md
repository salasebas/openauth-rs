# Upstream parity — openauth-tokio-postgres
...
| **Rust crate** | `crates/openauth-tokio-postgres/` |
| **Parity level** | **High** (~96% server-only for Postgres semantics this crate owns) |
| **Scope** | Server-side Postgres `DbAdapter`, migrations, and SQL rate-limit storage. Out of scope: HTTP routes (`openauth`, `openauth-core`), SQLx adapters (`openauth-sqlx`), pooling (`openauth-deadpool-postgres`), non-Postgres dialects, client SDKs |

## Summary

`openauth-tokio-postgres` implements the Better Auth Kysely Postgres adapter contract on a
single `tokio_postgres::Client`, sharing SQL planning with `openauth-sqlx`. CRUD, joins,
...
| Connection pooling | ➖ N/A | Single client by design; use `openauth-deadpool-postgres` for pools |
| HTTP routes | ➖ N/A | No route surface in this crate |
| Adapter factory ergonomics | ⚠️ Partial | Debug logs and dynamic transform hooks live in OpenAuth core/plugins |
...
| --- | --- | --- | --- |
| Postgres adapter integration | 39 `#[tokio::test]` in `tests/postgres_adapter.rs` | `adapter.kysely.pg.test.ts`, `adapter.kysely.custom-schema-pg.test.ts`, shared adapter suites, PG e2e | Requires `OPENAUTH_TEST_POSTGRES_URL` or Docker Compose defaults |
...
Quick count: `rg '#\[test\]|#\[tokio::test\]' crates/openauth-tokio-postgres` → **40** tests.
...
| Transaction callback | Generic return value from callback | `Result<(), OpenAuthError>` | Idiomatic Rust; DB state transitions preserved |
...
| Factory transforms | Runtime defaults, field maps, `onUpdate`, fallback joins in adapter factory | Applied in OpenAuth core/service layers above the adapter | Keep adapter focused on SQL execution |
...
| TPG-4 | Further parity needs core contract changes | Low | Gains beyond current ~96% may require shared OpenAuth adapter API work |
...
| `packages/kysely-adapter/src/query-builders.ts` | `src/query.rs` (via `openauth-sqlx` SQL planner) |
...
- [openauth-sqlx UPSTREAM.md](../openauth-sqlx/UPSTREAM.md) — shared SQL planner and multi-dialect context
- [openauth-deadpool-postgres UPSTREAM.md](../openauth-deadpool-postgres/UPSTREAM.md) — pooled Postgres variant
