...
pub fn sign_jwt<T>(payload: &T, secret: &str, expires_in: i64) -> Result<String, OpenAuthError>
where
    T: Serialize,
{
    let header = json!({ "alg": "HS256", "typ": "JWT" });
    let mut payload =
        serde_json::to_value(payload).map_err(|error| OpenAuthError::Crypto(error.to_string()))?;
    ...
    let header = URL_SAFE_NO_PAD.encode(
        serde_json::to_vec(&header).map_err(|error| OpenAuthError::Crypto(error.to_string()))?,
    );
    let payload = URL_SAFE_NO_PAD.encode(
        serde_json::to_vec(&payload).map_err(|error| OpenAuthError::Crypto(error.to_string()))?,
    );
...
pub fn verify_jwt<T>(token: &str, secret: &str) -> Result<Option<T>, OpenAuthError>
where
    T: DeserializeOwned,
{
...
fn sign_bytes(data: &[u8], secret: &str) -> Result<Vec<u8>, OpenAuthError> {
