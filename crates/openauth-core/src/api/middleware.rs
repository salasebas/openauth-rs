    ...
    EndpointMiddleware::new(move |context, request| {
        ...
        Box::pin(async move {
            let resource_id = request
                ...
                .ok_or(OpenAuthError::MissingPathParam {
...
fn unauthorized_response() -> Result<crate::api::ApiResponse, OpenAuthError> {
    json_response(
...
fn forbidden_response() -> Result<crate::api::ApiResponse, OpenAuthError> {
    json_response(
...
pub(crate) fn ensure_fresh_session(
    ...
    if age.whole_seconds() >= context.session_config.fresh_age as i64 {
        return Err(OpenAuthError::Api(error_codes::SESSION_EXPIRED.to_owned()));
    }
    Ok(())
}
