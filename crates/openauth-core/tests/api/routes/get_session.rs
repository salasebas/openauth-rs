...
use openauth_core::context::request_state::set_should_skip_session_refresh;
...
use openauth_core::plugin::{PluginBeforeHookAction, PluginSchemaContribution};
...
async fn get_session_route_reads_chunked_cookie_cache() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(cache_cookies
        .iter()
        .any(|cookie| cookie.name == "open-auth.session_data.1"));
    let cookie = format!(
        "{}; {}",
        signed_session_cookie("token_1")?,
        cookie_header(&cache_cookies)
    ...
    assert_eq!(
        body["session"]["user_agent"].as_str().map(str::len),
        Some(9000)
    );
    Ok(())
}
...
async fn get_session_route_filters_hidden_plugin_user_fields(
    ...
    assert_eq!(response.status(), StatusCode::OK);
    let body: Value = serde_json::from_slice(response.body())?;
    assert!(body["user"].get("tenant_id").is_none());
...
async fn get_session_route_returns_plugin_user_output_fields(
    ...
    assert_eq!(response.status(), StatusCode::OK);
    let body: Value = serde_json::from_slice(response.body())?;
    assert_eq!(body["user"]["tenant_id"], "tenant_1");
    Ok(())
}

fn hidden_plugin_user_field_options() -> OpenAuthOptions {
    OpenAuthOptions {
        plugins: vec![
...
fn plugin_user_field_options() -> OpenAuthOptions {
    OpenAuthOptions {
        plugins: vec![
            AuthPlugin::new("tenant").with_schema(PluginSchemaContribution::field(
                "user",
