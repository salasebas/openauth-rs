...
use openauth_core::db::{DbAdapter, DbRecord, DbValue, DeleteMany, User, Where};
...
fn reject_client_credentials_oidc_scopes(scopes: &[String]) -> Result<(), OpenAuthError> {
    let invalid = scopes
        .iter()
...
async fn create_refresh_token(input: CreateRefreshTokenInput<'_>) -> Result<String, OpenAuthError> {
    let iat = now();
