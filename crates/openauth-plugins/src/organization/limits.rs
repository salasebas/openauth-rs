...
pub type OrganizationLimitFuture =
    Pin<Box<dyn Future<Output = Result<bool, OpenAuthError>> + Send>>;
pub type OrganizationLimitCallback = Arc<dyn Fn(User) -> OrganizationLimitFuture + Send + Sync>;

pub type MembershipLimitFuture = Pin<Box<dyn Future<Output = Result<usize, OpenAuthError>> + Send>>;
...
impl MembershipLimit {
    pub async fn resolve(&self, context: MembershipLimitContext) -> Result<usize, OpenAuthError> {
        match self {
