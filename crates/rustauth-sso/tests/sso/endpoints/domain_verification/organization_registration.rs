...
#[tokio::test]
async fn register_provider_with_organization_id_requires_admin_role(
...
#[tokio::test]
async fn register_provider_with_organization_id_rejects_org_member(
) -> Result<(), Box<dyn std::error::Error>> {
    let (adapter, router) = router_with_options_and_extra_plugins(
        SsoOptions::default(),
        vec![AuthPlugin::new("organization")],
...
#[tokio::test]
async fn register_provider_with_organization_id_allows_org_admin(
) -> Result<(), Box<dyn std::error::Error>> {
    let (adapter, router) = router_with_options_and_extra_plugins(
        SsoOptions::default(),
        vec![AuthPlugin::new("organization")],
