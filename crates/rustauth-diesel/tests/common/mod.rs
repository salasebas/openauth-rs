...
pub fn preflight_error(
    ...
    RustAuthError::Adapter(format!(
        "{adapter} test database preflight failed for `{database_url}`: {error}. Ensure the Docker Compose service exists, the `rustauth` database exists, and the configured user has permissions. Override with RUSTAUTH_TEST_POSTGRES_URL or TEST_POSTGRES_URL when needed."
    ))
}
