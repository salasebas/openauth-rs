...
#[path = "../../../tests/support/diesel_migration_atomicity_postgres.rs"]
mod diesel_migration_atomicity;
...
use rustauth_diesel::{DieselPostgresAdapter, DieselPostgresRateLimitStore};
...
async fn verification_pool(max_connections: u32) -> Result<sqlx::PgPool, RustAuthError> {
    let database_url = database_url();
    PgPoolOptions::new()
        .max_connections(max_connections)
        .connect(&database_url)
        .await
        .map_err(|error| common::preflight_error("postgres verification", &database_url, error))
}

async fn connect_adapter(schema: DbSchema) -> Result<DieselPostgresAdapter, RustAuthError> {
    let database_url = database_url();
    DieselPostgresAdapter::connect_with_schema(&database_url, schema)
        .await
        .map_err(|error| common::preflight_error("diesel postgres", &database_url, error))
}
...
async fn adapter() -> Result<DieselPostgresAdapter, RustAuthError> {
...
async fn postgres_adapter_reports_public_capabilities() -> Result<(), RustAuthError> {
    let adapter = DieselPostgresAdapter::connect(&database_url())
        .await
        .map_err(|error| common::preflight_error("diesel postgres", &database_url(), error))?;
    ...
    assert_eq!(
        capabilities,
        AdapterCapabilities::new("diesel-postgres")
...
async fn postgres_rate_limit_store_rejects_invalid_rules_before_database_access(
    ...
    let adapter = DieselPostgresAdapter::connect(&database_url()).await?;
    let store = DieselPostgresRateLimitStore::from(&adapter);
...
async fn postgres_adapter_migration_plan_rolls_back_on_statement_failure(
    ...
    let schema = test_schema();
    let adapter = connect_adapter(schema.clone()).await?;
    diesel_migration_atomicity::assert_diesel_postgres_migration_plan_rolls_back(
        &adapter, &pool, &schema,
    )
    .await
}
...
async fn postgres_adapter_supports_schema_qualified_table_names() -> Result<(), RustAuthError> {
    ...
    sqlx::query(&format!("CREATE SCHEMA {schema_name}"))
        ...
        .await
        .map_err(sql_error)?;
    let adapter = connect_adapter(auth_schema.clone()).await?;
...
fn router(adapter: Arc<DieselPostgresAdapter>) -> Result<AuthRouter, RustAuthError> {
