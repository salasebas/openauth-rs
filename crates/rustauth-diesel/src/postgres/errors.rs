...
pub(super) fn inactive_transaction() -> RustAuthError {
    RustAuthError::Adapter("diesel postgres transaction is no longer active".to_owned())
}
...
pub(super) fn diesel_error_with_context(
    ...
    RustAuthError::Adapter(format!(
        "diesel postgres {operation} failed for SQL `{sql}` with {params} bound parameters: {error}"
    ))
}

pub(super) fn pool_error(error: impl std::fmt::Display) -> RustAuthError {
    RustAuthError::Adapter(format!("diesel postgres pool error: {error}"))
}
