...
use openauth_core::db::{auth_schema, AuthSchemaOptions, DbSchema};
...
impl DeadpoolPostgresBuilder {
    ...
    pub fn build_adapter(self) -> Result<DeadpoolPostgresAdapter, OpenAuthError> {
        self.build(NoTls)
    }

    pub fn build_adapter_tls<T>(self, tls: T) -> Result<DeadpoolPostgresAdapter, OpenAuthError>
    where
    ...
    pub async fn connect(self) -> Result<DeadpoolPostgresAdapter, OpenAuthError> {
    ...
    pub async fn connect_tls<T>(self, tls: T) -> Result<DeadpoolPostgresAdapter, OpenAuthError>
    where
        T: MakeTlsConnect<Socket> + Clone + Sync + Send + 'static,
        T::Stream: Sync + Send,
        T::TlsConnect: Sync + Send,
    ...
    fn build<T>(self, tls: T) -> Result<DeadpoolPostgresAdapter, OpenAuthError>
    where
        T: MakeTlsConnect<Socket> + Clone + Sync + Send + 'static,
        T::Stream: Sync + Send,
        T::TlsConnect: Sync + Send,
...
impl DeadpoolPostgresStores {
    ...
    pub fn apply_to_options(
        &self,
        options: openauth_core::options::OpenAuthOptions,
    ) -> openauth_core::options::OpenAuthOptions {
        use openauth_core::options::RateLimitOptions;
        options.rate_limit(RateLimitOptions::database(self.rate_limit.clone()))
    }

    pub fn adapter(&self) -> std::sync::Arc<dyn openauth_core::db::DbAdapter> {
...
impl DeadpoolPostgresBuilder {
    pub async fn build_stores(self) -> Result<DeadpoolPostgresStores, OpenAuthError> {
    ...
    pub async fn build_stores_tls<T>(self, tls: T) -> Result<DeadpoolPostgresStores, OpenAuthError>
    where
        T: MakeTlsConnect<Socket> + Clone + Sync + Send + 'static,
        T::Stream: Sync + Send,
        T::TlsConnect: Sync + Send,
