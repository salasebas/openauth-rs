# Upstream parity — openauth-deadpool-postgres
...
| **Rust crate** | `crates/openauth-deadpool-postgres/` |
| **Parity level** | **High (~96%)** for observable Postgres adapter semantics |
| **Scope** | Server-side pooled Postgres `DbAdapter`, `RateLimitStore`, and migration execution. SQL planning and dialect logic live in [`openauth-sqlx`](../openauth-sqlx/UPSTREAM.md) and [`openauth-tokio-postgres`](../openauth-tokio-postgres/README.md). HTTP routes, adapter factory transforms, and client SDKs are out of scope. |

## Summary

`openauth-deadpool-postgres` is the recommended production Postgres adapter: it wraps
`deadpool-postgres` pooling around shared Postgres SQL planning from `openauth-sqlx`
and driver helpers from `openauth-tokio-postgres`. Observable database behavior
...
| Nested transactions | ❌ Missing | Rejected with adapter error (no savepoints); consistent with other OpenAuth Postgres adapters |
| Schema creation | ✅ High | OpenAuth and plugin tables from `DbSchema`; optional compiled SQL output |
| Migration planning | ✅ High | Additive tables, columns, indexes, FKs, generated defaults; type-mismatch reporting |
| Migration execution | ✅ High | Each plan runs in a single Postgres transaction; matches `PostgresAdapter` in `openauth-sqlx` |
...
| Public HTTP routes | ➖ N/A | Kysely adapter has no route surface; routes live in `openauth-core` / `openauth` |
| Adapter factory transforms | ⚠️ Partial | Defaults, field mappings, and `onUpdate` live in OpenAuth core/service layers |
| Non-Postgres dialects | ➖ N/A | SQLite, MySQL, MSSQL, D1, etc. are [`openauth-sqlx`](../openauth-sqlx/UPSTREAM.md) scope |
...
| **Total (this crate)** | **49** | **Postgres Kysely + shared suites** | `cargo nextest list -p openauth-deadpool-postgres` |

Verify:

```bash
cargo nextest run -p openauth-deadpool-postgres
```

Requires reachable Postgres (`OPENAUTH_TEST_POSTGRES_URL` or Docker Compose defaults on
...
| Delete semantics | Full `WHERE` passed through `delete` | Single-row `delete`; bulk via `delete_many` | Fail-closed auth boundaries (shared with [`openauth-sqlx`](../openauth-sqlx/UPSTREAM.md)) |
...
| `query-builders.ts` | Shared SQL planner via `openauth-sqlx` / `openauth-tokio-postgres` |
...
- [openauth-sqlx UPSTREAM](../openauth-sqlx/UPSTREAM.md) — shared SQL planning and dialect gaps
- [openauth-tokio-postgres](../openauth-tokio-postgres/README.md) — non-pooled Postgres sibling
