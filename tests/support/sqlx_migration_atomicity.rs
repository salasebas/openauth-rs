//! Dialect-specific migration rollback assertions for SQLx adapter integration tests.
//! Each function is gated by the matching `openauth-sqlx` feature.
...
pub async fn assert_postgres_migration_plan_rolls_back(
    adapter: &openauth_sqlx::PostgresAdapter,
    pool: &sqlx::PgPool,
    schema: &DbSchema,
...
pub async fn assert_mysql_migration_plan_rolls_back(
    adapter: &openauth_sqlx::MySqlAdapter,
    pool: &sqlx::MySqlPool,
    schema: &DbSchema,
) -> Result<(), OpenAuthError> {
    let plan = adapter.plan_migrations(schema).await?;
